...
#define X_LINE_START       27
#define X_LINE_END         35           
...
void x_step_ISR(){             // Interup Fuction 0 pin D2; Motor X
  ...
  if(RunFlag && IntFlag && (x_cuttent_time - x_debounce_time > 100)) {
    x_count =+STEPS;               //x_count + 82;
  }
  x_debounce_time = e_cuttent_time;
}

void y_step_ISR(){              // Interup Fuction 1 pin D3; Motor Y
  y_cuttent_time = millis();
  if (RunFlag && IntFlag && (y_cuttent_time - y_debounce_time > 100)) {
    y_count =+STEPS;
  }
  y_debounce_time = e_cuttent_time;
}

void e_step_ISR(){              // Interup Fuction 4 PIN 19; Motor E
  e_cuttent_time = millis();
  if (RunFlag && IntFlag && (e_cuttent_time - e_debounce_time > 100)) {  
    e_count =+STEPS;
  }
...
void q_step_ISR(){             // Interup Fuction 3 pin D20; Motor Q
  ...
  if (RunFlag && IntFlag && (q_cuttent_time - q_debounce_time > 100)) {
    q_count =+STEPS;
  }
  q_debounce_time = e_cuttent_time;
}
